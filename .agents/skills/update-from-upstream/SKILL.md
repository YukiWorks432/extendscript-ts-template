...
| `.agents/skills/add-script/`           | add-script スキル                  |
| `.agents/skills/debug-script/`         | debug-script スキル                |
| `.agents/skills/es-to-ts/`             | es-to-ts スキル                    |
| `.agents/skills/update-from-upstream/` | update-from-upstream スキル        |
| `docs/`                                | ドキュメント                       |
...
利用可能な質問ツールまたは簡潔な日本語の質問で以下を確認する:
...
- `.agents/skills/` 配下にユーザー独自スキルがある場合は、上記 4 スキル以外を上書きしない
