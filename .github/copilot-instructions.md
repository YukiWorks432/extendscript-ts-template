...
- そのネストは避けられませんか？早期リターンなどでネストを浅くすることを心掛けてください。
- 変数名、関数名、クラス名は具体的かつ説明的にしてください。略語や省略形は避けてください。
- コメントは必要最低限にとどめ、コード自体が自己説明的であることを優先してください。
- 冗長なコードや重複コードは避け、共通化できる部分は関数やクラスにまとめてください。
- entry 関数により、全体的なエラーハンドリングはできています。 適切にthrowを使用してかまいません。
- 大域脱出をする場合は throw new Error("message") を使用してください。
- After Effects特有の型　（AVLayer、TextLayerなど）を判定する場合は lib の isXXX 型ガード関数を使用してください。存在しなければ適切に作成してください。
